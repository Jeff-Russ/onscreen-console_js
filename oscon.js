...
window.init_on_screen_console = function(){
   ...
   window.show_on_screen_console();
   window.enable_osc_keys;
};
...
window.on_screen_console = function(bool){
   window.on_screen_console_bool = bool;
   if (window.on_screen_console_bool) {
   ...
   else $('#on_screen_console').remove();
};

// make console messages visible:
window.show_on_screen_console = function(){ 
...
// hide (but not disable) on screen console:
window.hide_on_screen_console = function(){ 
   $('#on_screen_console').css('visibility','hidden');
};

// clear console:
window.clear_on_screen_console = function(){
   window.set_on_screen_console(false);
   window.set_on_screen_console(true);
};
   ...
   if (bool){
      ...
      $(document).keydown( function(e) {
         if (e.keyCode in map) { 
            ...
            if (map[192] && map[68]) { 
               window.set_on_screen_console(false); // ( `d ) disable
            }
            if (map[192] && map[83]) { 
               window.show_on_screen_console();     // ( `s ) show
            }
            if (map[192] && map[72]) { 
               window.hide_on_screen_console();     // ( `h ) hide
            } 
            if (map[192] && map[67]) { 
